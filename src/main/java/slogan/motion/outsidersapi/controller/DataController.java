...
public class DataController {
    ...
    // player id - arena id
    private final Map<Integer, Integer> privateMatchMaking = new HashMap<>();

    // player id - level
    private final Map<Integer, Integer> publicMatchMaking = new HashMap<>();

    // player id - level
    private final Map<Integer, Integer> ladderMatchMaking = new HashMap<>();

    // player id - arena id
    private final Map<Integer, Integer> stagedGames = new HashMap<>();
    ...
    public int getArenaForPlayer(@PathVariable int playerId, @PathVariable String opponentName) {
        ...
        Player me = this.IPlayerService.findById(playerId).get();

        if (opponent == null) {
            log.warn("------>>PRIVT|Player:{} opponent with name {} does not exist", me, opponentName);
            return 0;
        }

        if (this.getPrivateMatchMaking().get(opponent.getId()) != null) {
            int arenaId = this.removeFromPrivateMatchMaking(opponent.getId());
            log.info("{}>>PRIVT|Player:{} found opponent {}", arenaId, me, opponent);
            return arenaId;
        } else {
            // TODO: standardize this ArenaID assignment
            int randomNum = ThreadLocalRandom.current().nextInt(10000000, 99999999);
            this.addToPrivateMatchMaking(playerId, randomNum);
            log.info("{}>>PRIVT|Player:{} waiting on opponent {}", randomNum, me, opponent);
            return randomNum;
        }
    }
    ...
    public int getLadderMatchArena(@PathVariable int playerId, @PathVariable int playerLevel) throws Exception {
        ...
        while (waiting) {
            log.info("------>>LADDR|playerId:{} waiting", playerId);
            if (seconds > 30) {
            ...
            Thread.sleep(1000);
            seconds++;
            log.trace("------>>LADDR|" + seconds + " seconds have passed");
            ...
            if (found != 0) {
                ...
                // todo: Standardize this arenaID generation
    ...
    public int getQuickMatchArena(@PathVariable int playerId, @PathVariable int playerLevel) throws Exception {
        ...
        while (waiting) {
            log.info("------>>QUICK|playerId:{} waiting", playerId);
            if (seconds > 30) {
                // if someone doesn't connect in 30 seconds, reply with "empty queue" (0) or
                // throw http exception
                output = 0;
            ...
            Thread.sleep(1000);
            seconds++;
            log.trace("------>>QUICK|" + seconds + " seconds have passed");
