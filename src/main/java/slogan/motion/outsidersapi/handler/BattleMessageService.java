...
public class BattleMessageService {
	...
	public String handleMatchmakingMessage(Map valueMap) throws JsonProcessingException {
		...
		Integer playerId = matchMakingDTO.getPlayerId();
		Player player = this.IPlayerService.findById(playerId).get();
		Integer arenaId = matchMakingDTO.getArenaId();
		...
		if (battle == null) {
			...
			i2.setPosition(1);
			i3.setPosition(2);
			player.addICombatants(List.of(i1,i2,i3));
...
//		LOG.info("Cost Check");
		Integer playerId = energyTradeDTO.getPlayerId();
		Integer arenaId = energyTradeDTO.getArenaId();
		Battle b = IBattleService.getByArenaId(arenaId);
		...
		if (chosenAbilities != null) {
			for (TargetCheckDTO targetCheckDto : chosenAbilities) {
				Ability ability = targetCheckDto.getAbility();
...
//		LOG.info(responseJson.toString());
	...
	public String handleGameEndMessage(Map valueMap) {
		...
		Player newLoser = this.IPlayerService.create(loser);
		Player newWinner = this.IPlayerService.create(winner);
	...
	public Integer[] checkIfStunned(Integer[] input, List<Combatant> team) {
		for (int i = 0; i < 3; i++) {
			...
			if (stunned) {
				...
				if (physStunned) {
					for (Ability a : character.getIAbilities()) {
						if (a.isPhysical()) {
