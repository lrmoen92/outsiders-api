...
public class TurnResolver {
	...
	private static List<TargetCheckDTO> moves = new ArrayList<>();
	private static List<Effect> effects = new ArrayList<>();
	private static boolean isPlayerOne = false;

	private static List<Effect> currentTargetEffects = new ArrayList<>();
	private static List<Effect> currentFromEffects = new ArrayList<>();
	...
	private static TurnEndDTO turnEndDTO = null;

	private static Effect myCounter = null;
	private static Effect theirCounter = null;
	private static Effect ourCounter = null;
	...
	private static boolean isOriginCounterTriggered(Effect counter, Ability a) {
	...
	private static boolean isTargetCounterTriggered(Effect counter, Ability a) {
		boolean counterIsTriggered = true;
		if (counter.isConditional()) {
			counterIsTriggered = false;
			String flagName = counter.getCondition().split("_")[2];
	...
	private static boolean isNewEffect(Effect e) {
		return e.getInstanceId() <= 0;
	}

	private static boolean isEnemyEffect(Effect e) {
		return ((isPlayerOne && e.getOriginCharacter() > 2) || (!isPlayerOne && e.getOriginCharacter() < 3));
	}

	private static boolean isHiddenEnemyEffect(Effect e) {
	...
	private static void extractBattle(Battle incomingBattle, TurnEndDTO dto, boolean playerOne) {
	...
	private static void extractValues(Effect currentEffect, Combatant tarChar, Combatant fromChar,
	...
	private static void resetFlagsAndRemoveOldEffects() {
		if (!isPlayerOne) {
			for (Combatant c : battle.getPlayerOneTeam()) {
				...
				List<Effect> currentEffects = c.getEffects();
				currentEffects.removeIf((Effect e) -> {
	...
	public Battle handleTurns(Battle incomingBattle, TurnEndDTO dto, boolean playerOne) throws Exception {
			...
			if (a.isEnemy()) {
				...
				if (!counterIsTriggered) {
					a.getAoeEnemyEffects().forEach(e -> {
						applyEffectToCharacters(new Effect(e), enemyTeam, self, targetPositions, true);
					});
				}
			}
			if (a.isAlly()) {
				...
				if (!counterIsTriggered) {
					a.getAoeAllyEffects().forEach(e -> {
						applyEffectToCharacters(new Effect(e), team, self, targetPositions, true);
					});
				}
			}
			if (a.isSelf()) {
				...
				if (!counterIsTriggered) {
					a.getSelfEffects().forEach(e -> {
						applyEffectToCharacter(new Effect(e), self, self, false, true);
					});
				}
			}
		} else {
	...
	private static void applyOngoingEffects(Effect e, boolean hidden) {
	...
	private static void applyEffectToCharacters(Effect effect, List<Combatant> characters,
	...
	private static void applyEffectToCharacter(Effect effect, Combatant targetCharacter, Combatant fromCharacter,
	...
	private static void applyEffectToCharacter(Effect incomingEffect, Combatant targetCharacter,
	...
	private static void checkIfDamagable() {
		for (Effect e : currentFromEffects) {
		...
		for (Effect e : currentTargetEffects) {
	...
	private static void checkForReveal() {
		if (isReveal) {
			for (Effect effectEnded : currentTargetEffects) {
	...
	private static void removeShields() {
		if (!isHeal && isDamagable && !isAffliction) {
			for (Effect ef : currentTargetEffects) {
	...
	private static void progressOldEffect() {
		int index = -1;
		int counter = 0;
		for (Effect oldEffect : currentTargetEffects) {
		...
		if (index == -1) {
			...
			if (hiddenPass && !effect.isVisible()) {
				...
				if (dur == 0) {
					// their hidden effect is ending, remove it as normal but replace it with a 1
					// turn blank effect that's not hidden
					Effect effectEnded = currentTargetEffects.get(index);
					effectEnded.setQuality(null);
					effectEnded.setStatMods(new HashMap<>());
					effectEnded.setCondition(null);
					effectEnded.setConditional(false);
